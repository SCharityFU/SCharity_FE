...
import { ArrowLeft, Landmark, CreditCard, User, Search, ChevronDown, CheckCircle2, Loader2 } from 'lucide-react';
...
import { useGetMyRequestByIdQuery, useUpdateRequestBankInfoMutation } from '@/lib/store/features/campaign/campaignApi';
...
export default function UpdateBankInfoPage() {
  ...
  useEffect(() => {
    ...
    const { bankName, accountNumber: existingAccNum, accountHolderName: existingHolder } = requestData.data.bankInfo;
    ...
    <div className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-10">
          ...
          <p className="text-black/50 text-sm mt-2">Chỉnh sửa thông tin tài khoản ngân hàng cho yêu cầu chiến dịch</p>
        </div>
          ...
          <>
            ...
            <div className="glass-card rounded-2xl p-8 space-y-6">
                    ...
                    >
                      ...
                      <div className="max-h-56 overflow-y-auto">
                        {filteredBanks.length === 0 ? (
                          <p className="text-center text-black/30 text-sm py-6">Không tìm thấy ngân hàng</p>
                        ) : (
              ...
              <div>
                ...
                />
                <p className="text-xs text-black/30 mt-1.5">Tự động viết hoa, không dấu (theo chuẩn ngân hàng)</p>
              </div>

              {/* Actions */}
              <div className="flex items-center gap-3 pt-4 border-t border-black/5">
                <Button onClick={handleConfirmOpen} disabled={!isFormValid || isLoading} className="flex-1">
                  {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />}
                  Xác Nhận Lưu
                </Button>
                <Button variant="outline" onClick={() => router.push('/dashboard/my-requests')} disabled={isLoading}>
                  Hủy
                </Button>
              </div>
            </div>
