  ...
  return (
    <Link href={`/campaigns/${campaign.id}`} className="block group">
      <div className="glass-card rounded-2xl overflow-hidden hover:border-black/20 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-black/30">
        {/* Image area */}
        <div
          className={`h-48 ${campaign.imageGradient} relative flex items-center justify-center overflow-hidden`}
        >
          <span className="text-6xl z-10">{campaign.emoji}</span>
          <div className="absolute inset-0 bg-black/20" />
          {/* Category badge */}
          <span className="absolute top-3 left-3 px-2.5 py-1 rounded-full text-xs font-medium glass text-black">
            {campaign.category}
          </span>
          {/* Like button */}
          <button
            className="absolute top-3 right-3 w-8 h-8 rounded-full glass flex items-center justify-center hover:bg-black/20 transition-colors"
            onClick={(e) => {
              e.preventDefault();
              setLiked(!liked);
            }}
        ...
        {/* Content */}
        <div className="p-5">
          <h3 className="font-semibold text-base text-black mb-2 line-clamp-2 group-hover:text-rose-300 transition-colors">
            <HighlightText
              variant={campaign.highlight || "underline"}
              color="primary"
              animate
              className="text-black group-hover:text-rose-300"
            >
              {campaign.title}
            </HighlightText>
          </h3>
          <p className="text-sm text-black/50 line-clamp-2 mb-4">{campaign.description}</p>
