...
export function CampaignCard({ campaign }: { campaign: CampaignDto }) {
  ...
  const goalDisplay = formatAmountByMagnitude(campaign.goalAmount || 0);
          ...
          <div className="flex items-center justify-between text-xs text-black/50">
            <div className="flex items-center gap-1">
              <span className="font-bold text-black text-sm">
                <NumberCounter
                  value={raisedDisplay.value}
                  prefix="₫"
                  suffix={raisedDisplay.unit}
                  decimals={raisedDisplay.decimals}
                  duration={1.5}
                />
              </span>
              <span>/ {goalDisplay.formatted}</span>
            </div>
