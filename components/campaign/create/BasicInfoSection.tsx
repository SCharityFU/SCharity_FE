...
import { removeDiacritics } from '@/components/campaign/create/utils';
import type { VietQRBank } from '@/hooks/useVietQRBanks';
import { MAXIMUM_CAMPAIGN_GOAL, MINIMUM_CAMPAIGN_GOAL } from './constants';
...
function TitleField({ titleMax }: { titleMax: number }) {
  const { register, control } = useFormContext<CreateCampaignFormValues>();
    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        <input
          ...
          className="w-full pl-4 pr-14 py-3 rounded-xl glass border border-black/10 text-black placeholder-black/30 outline-none focus:border-rose-500/50 transition-colors text-sm"
        />
...
function GoalAmountField({ goalPresets, shortVND }: { goalPresets: number[]; shortVND: (value: number) => string }) {
  const { control, setValue, trigger } = useFormContext<CreateCampaignFormValues>();
  const goalAmount = useWatch({ control, name: 'goalAmount' }) ?? 0;
  const [goalRaw, setGoalRaw] = useState('');

  useEffect(() => {
    setGoalRaw(goalAmount > 0 ? formatVNDInput(goalAmount) : '');
  }, [goalAmount]);
    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        <input
          ...
          value={goalRaw}
          onChange={(e) => {
            const num = parseVNDInputToNumber(e.target.value);
            const clamped = Math.min(num, MAXIMUM_CAMPAIGN_GOAL);
            setValue('goalAmount', clamped, { shouldValidate: true, shouldDirty: true });
            setGoalRaw(clamped > 0 ? formatVNDInput(clamped) : '');
          }}
          onBlur={() => {
            void trigger('goalAmount');
          }}
          min={MINIMUM_CAMPAIGN_GOAL}
          max={MAXIMUM_CAMPAIGN_GOAL}
        ...
        {goalPresets.map((preset) => (
          <button
            ...
            onClick={() => {
              ...
              setGoalRaw(formatVNDInput(preset));
            }}
...
function CategoryField({ categoryLabels }: { categoryLabels: Record<CampaignCategory, string> }) {
...
const BankInfoSection = memo(function BankInfoSection({
  bankSearch,
  onBankSearchChange,
  bankDropdownOpen,
...
export function BasicInfoSection({
  ...
  return (
    <div className="space-y-6">
      <TitleField titleMax={titleMax} />

      <GoalAmountField goalPresets={goalPresets} shortVND={shortVND} />

      <DeadlineField minDeadline={minDeadline} maxDeadline={maxDeadline} formatDateVN={formatDateVN} />

      <CategoryField categoryLabels={categoryLabels} />

      <BankInfoSection
        bankSearch={bankSearch}
        onBankSearchChange={onBankSearchChange}
        bankDropdownOpen={bankDropdownOpen}
        onBankDropdownOpenChange={onBankDropdownOpenChange}
