...
export function WithdrawRequestModal({ open, onClose, campaign }: WithdrawRequestModalProps) {
  ...
  const [amount, setAmount] = useState<number>(campaign.raisedAmount ?? 0);
  ...
  const { data: bankChangeStatusData } = useGetBankChangeRequestStatusQuery(
    activeBank?.id ?? "",
  ...
  const handleProceed = () => {
    ...
    if (amount <= 0 || amount > raised) {
      setErrorMsg("Số tiền rút không hợp lệ.");
      return;
    }
    setErrorMsg(null);
    setStep("confirm");
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 rounded-xl p-4 border border-black/5">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-[11px] text-black/40 font-medium uppercase tracking-wider">
                    Số dư có thể rút
                  </p>
                </div>
                <p className="text-2xl font-black text-black tabular-nums">{formatVND(raised)}</p>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    max={raised}
                    min={0}
                    className="w-full py-3 px-4 pr-12 rounded-xl bg-gray-50 border border-black/10 text-sm font-bold text-black tabular-nums focus:outline-none focus:ring-2 focus:ring-rose-500/30 focus:border-rose-500/40 transition-all"
                  />
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs text-black/30 font-medium">
                    VND
                  </span>
                </div>
              </div>
              ...
              {activeBank && !editingBankInfo && !bankChangePending && bankStatus && !hasPendingRequest && (
                <button
                  ...
                  onClick={() => {
                    ...
                    const foundBank = bankList?.find((b) => b.shortName.toLowerCase() === activeBank.bankName.toLowerCase());
                    ...
                    >
                      {selectedVietQrBank ? (
                        <>
                          ...
                          <img src={selectedVietQrBank.logo} alt={selectedVietQrBank.shortName} className="w-5 h-5 object-contain rounded" />
                      ...
                      <ChevronDown className={`w-3.5 h-3.5 text-black/40 transition-transform ${bankDropdownOpen ? "rotate-180" : ""}`} />
                    </button>
                        ...
                        >
                          ...
                          <div className="overflow-y-auto flex-1">
                            {filteredBanks.map((bank) => (
                              ...
                              >
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img src={bank.logo} alt={bank.shortName} className="w-6 h-6 object-contain rounded bg-white p-0.5 border border-black/5" />
                                <div className="flex-1 min-w-0">
              ...
              {(bankChangePending || !bankStatus || hasPendingRequest) && (
                <div className="flex items-start gap-2 p-3 rounded-xl bg-amber-500/8 border border-amber-500/20">
                  ...
                  <div className="flex-1">
                    <p className="text-[11px] font-bold text-amber-800">Yêu cầu thay đổi thông tin ngân hàng đang chờ duyệt</p>
                    <p className="text-[11px] text-amber-700 mt-0.5">
                      Bạn sẽ nhận email thông báo khi yêu cầu được duyệt. Trong thời gian này, bạn không thể tạo thêm yêu cầu đổi thông tin (đã có 1 yêu cầu gửi lúc {bankChangeStatusData?.latestRequest?.createdAt ? new Date(bankChangeStatusData.latestRequest.createdAt).toLocaleString("vi-VN") : "mới đây"}). Khi admin duyệt, tính năng rút tiền sẽ mở lại.
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="bg-gray-50 rounded-xl p-3 border border-black/5 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p
                    className={cn(
                      "text-[11px]",
                      isKycVerified ? "text-emerald-600" : "text-amber-600",
                    )}
                  >
                    {isKycVerified
                      ? "Đã xác thực"
                      : "Chưa xác thực — Vui lòng xác thực trước khi rút tiền"}
                  </p>
                </div>
                {!isKycVerified && (
                  <Link
          ...
          {step === "confirm" && activeBank && (
            <div className="space-y-4">
              <div className="text-center py-2">
                <p className="text-sm text-black/60">Bạn xác nhận rút:</p>
                <p className="text-2xl font-black text-black mt-1 tabular-nums">
                  {formatVND(amount)}
                </p>
          ...
          {step === "success" && (
            <div className="text-center py-6">
              ...
              <p className="text-xs text-black/40 mt-1.5 max-w-xs mx-auto">
                Yêu cầu rút <span className="font-bold text-black">{formatVND(amount)}</span> đang
                chờ Admin xem xét. Bạn sẽ nhận email thông báo khi có kết quả.
              </p>
          ...
          {step === "form" && (
            <>
              ...
              <button
                ...
                disabled={!activeBank || !isKycVerified || amount <= 0 || bankChangePending || editingBankInfo || !bankStatus || hasPendingRequest}
