...
const NumberCounter = React.forwardRef<HTMLSpanElement, NumberCounterProps>(
    (
        ...
        React.useEffect(() => {
            ...
            const animate = () => {
                const now = Date.now();
                if (now < startTime) { animationFrame = requestAnimationFrame(animate); return; }
                if (now >= endTime) { setCount(value); return; }
        ...
        const formatted = formatFn ? formatFn(count) : formatNumber(count, decimals, separator, decimalSeparator);
        return (
...
export type { CircularCounterProps, EasingType, NumberCounterProps, RollingCounterProps, StatCounterProps };
